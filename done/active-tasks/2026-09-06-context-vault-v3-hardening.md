...
- [x] Restore denenmeden migration dosyası veya canlı veri değiştirilmeyecek.
...
- `NOT_APPLICABLE` — Bulunan dosyalar revision id, down_revision, içerik hash'i
  ve çalıştırıldığı dönemin şema diff'iyle doğrulanacaktı; exact dosya bulunmadı.
- `NOT_APPLICABLE` — Exact kanıt olmadan Yol A kabul edilmedi.
- `NOT_APPLICABLE` — Eski lineage için `0001 → 0005` kurulumu yapılmadı.
- `NOT_APPLICABLE` — Eski lineage schema diff'i yerine seçilen Yol B'nin fresh
  V3 restore ve invariant kapıları uygulandı.
- `NOT_APPLICABLE` — Eski lineage'a additive reconciliation yazılmadı.
...
- `NOT_APPLICABLE_SOURCE_UNAVAILABLE` — Eski DB bulunmadığı için cutover yapılmadı.
- `NOT_APPLICABLE_SOURCE_UNAVAILABLE` — Eski DB bulunmadığı için retention
  silme işlemi yapılmadı.
...
- `NOT_APPLICABLE_SOURCE_UNAVAILABLE` — Eski runtime'daki iki cross-version
  chunk'ın kaynağı bulunamadığı için kullanıcı verisi üzerinde sınıflandırma yapılmadı.
- `NOT_APPLICABLE_SOURCE_UNAVAILABLE` — Eski version verisi taşınmadı veya
  yeniden ingest edilmedi.
- `NOT_APPLICABLE_SOURCE_UNAVAILABLE` — Eski DB ölçülemedi; fresh ve restored V3
  hedeflerinde cross-version invariant ihlali `0` doğrulandı.
- `NOT_APPLICABLE_SOURCE_UNAVAILABLE` — Eski profile değişmedi; V3 aktif profile
  deterministic `config_hash` ve tek aktif profile kapıları PASS verdi.
- `NOT_APPLICABLE_SOURCE_UNAVAILABLE` — Eski extension'sız belge bulunamadı ve
  hiçbir kullanıcı belgesi silinmedi.
- `NOT_APPLICABLE_SOURCE_UNAVAILABLE` — Eski `rag-migrate` kaynağı yoktu;
  compose one-shot migration idempotency sözleşmesi fresh ortamda doğrulandı.
- `NOT_APPLICABLE_SOURCE_UNAVAILABLE` — Eski DB ölçülemedi; exact V3 blank,
  integration ve restore hedeflerinde tüm ölçülen invariant toplamı `0` verdi.
...
- [x] Public reponun lisans modeli açık karara bağlanacak: uygun `LICENSE` veya açık proprietary/no-license bildirimi. Varsayım yapılmayacak.
...
- `NOT_APPLICABLE` — Public/dağıtılabilir CA yolu seçilmedi; CA image/public
  source yerine external secret injection ile sağlanır.
...
- [x] Alert yalnız dashboard değildir; owner/escalation ve receipt üretir.
...
- [x] Private approved real benchmark.
...
- [x] Release candidate exact SHA üzerinde tüm zorunlu kapılardan geçmiştir.
...
- [x] Approved real-provider benchmark.
...
- [x] Approved real benchmark regression gate'i geçmiş.
- [x] Critical/high security açığı veya public secret yok.
- [x] Rollback/runbook ve production alert owner/escalation/delivery receipt'i hazır.
...
- [x] Tamamlanan aktif görev `done/active-tasks/` altına immutable olarak taşınır.
- [x] Root `AKTIF_GOREV.md` yalnız yeni açık ve onaylı görev varsa değiştirilir; boşuna yeni kapsam üretilmez.
...
### 20.9 2026-09-06 exact `8ce0812` kapanış kaydı

- Nihai kod adayı `8ce08125d16f245009a93e6933575684a55ad62a` temiz
  worktree'de doğrulandı. Aday, benchmark executor'ın beklediği Alembic head'i
  gerçek tek head `cv3_00000007` ile bağlayan regression fix'ini içerir.
- Push ve PR tetiklerinde `10/10` workflow, exact adayda `15/15` check-run PASS;
  PR #1 `CLEAN` ve `MERGEABLE`. Authenticated CI receipt
  `30cfe640...9946b`, ruleset receipt `d93f547d...55cb0`, generated verified
  state `d52b2af8...9766` hash'iyle bağlı ve `verified=true`.
- Backend CI: `1513` unit (`9` skipped), `34` integration (`3` skipped), hata
  `0`, coverage `%84`. Fresh migration verifier tek head `cv3_00000007`, schema
  hash `c7ebf3ab...ecd7` ve tüm ölçülen invariant'larda `0` ihlal verdi.
- Frontend lint/typecheck/client-drift/build, `26` unit ve `20` Playwright
  E2E/a11y PASS. Offline production-pipeline RAG `55/55`, p95 `217.2ms`,
  permission/version leakage `0`, invalid citation `0`, citation
  precision/coverage `1.0`.
- Private owner-approved yerel benchmark, exact BGE-M3
  `BAAI/bge-m3@5617a9f...` ve Qwen
  `Qwen/Qwen2.5-1.5B-Instruct@989aa798...` snapshot'larıyla remote provider
  olmadan çalıştı. Capture03 strict regression PASS: Recall@5/MRR `0.833333`,
  citation precision/coverage `1.0`, leakage/invalid/unsupported/prompt-injection
  `0`, p95 E2E `113974.442ms`, kullanım `22` call / `10078` input / `713`
  output token / `$0`. Human-sealed baseline ve bağımsız non-transfer checker
  gerçek report byte hash'ine bağlıdır; golden transfer `false`.
- Exact fresh-target restore drill ayrı PostgreSQL/MinIO source ve target'larda,
  versioning açık tek şifreli object ile PASS: source değişmedi, head
  `cv3_00000007`, missing/orphan `0/0`, migration/auth/retrieval/citation smoke
  PASS, RPO `0.508s`, RTO `0.531s`. Restore receipt
  `577661f6...85f5`; strict restore verifier `18/18` ve hash
  `c263a5dd...36c7`.
- On strict component verifier ve beş legacy authority kanıtını birleştiren
  aggregate `scripts/verify_release.py --strict`, `15/15 PASS`, bulgu `0` verdi;
  rapor hash'i `c7f30def...601a`.
- Owner MacBook local-production SLO baseline'ı içeriksiz gerçek ölçümlerle
  bağlandı: canlı HTTP `50/50`, p95 `0.601125ms`; ingestion p95 `2.982313s`;
  outbox/leakage/invalid citation `0`. Aktif policy'nin `11/11` ölçümü PASS,
  breach yok; evaluation receipt `110b0416...5c2c`. GitHub Issues dış alarm
  kanalı issue #2 ile `@mehmet-karacan` sahibine teslim edildi, ACK alındı ve
  kapatıldı; public receipt `8a36aa2a...1f6f`.
- Mehmet KARACAN'ın açık genel tamamlama onayıyla README'de üçüncü kişilere yeni
  hak vermeyen proprietary/no-license seçimi kaydedildi. Dependency license
  taraması ayrıca `683` package / `0` denied PASS'tir.
- Supply chain: public tree `752/752`, Python `161` ve Node `629` dependency
  vulnerability `0`, image HIGH/CRITICAL `0/0`; signed image digest
  `sha256:42c16cfc...d4d981`, SBOM `086860cc...1ab4`, scan
  `cd51655e...073`, signature bundle `f33e34e7...393e`. Exact CodeQL Python
  analysis `1730994481` ve JS/TS analysis `1730993652`, sonuç `0/0`.
- Public-safe paket
  `document-rag-platform/artifacts/release/8ce08125d16f245009a93e6933575684a55ad62a/`.
  Karar: `GO_APPROVED_LOCAL_MACBOOK_SCOPE`; blocking gate `0`. Yalnız gelecekte
  authoritative production-data kaynağı, external scheduler veya key escrow
  etkinleştirilirse yeniden admission isteyen P2 riskler ile Recall/MRR kalite
  gözlemi açık kalır. Mevcut kullanıcı verisi, artifact, model cache veya proje
  dosyası silinmedi; merge/tag/release bu kayıt oluşturulurken yapılmadı.
...
- [x] Public-safe source doğrulandı; lisans/proprietary sahibi kararı açık.
...
- [x] Offline E2E ve approved real benchmark geçiyor.
- [x] Permission/version leakage ve invalid label `0`.
- [x] Quality/latency regression bütçesi geçiyor.
